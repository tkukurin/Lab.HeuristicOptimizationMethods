...
public class Evaluator {
    ...
    // TODO evaluator rates how good the solution is
        ...
        System.out.println("First goal: " + firstGoal);
        System.out.println("Second goal: " + secondGoal);
    return 0.0;
  }
    ...
    private double p1f1() {
        ...
        //System.out.println("p1f1=" + Math.pow(solutionInstance.getNumOfUsedTracks() - 1, -1) * (double) f1);
        return Math.pow(solutionInstance.getNumOfUsedTracks() - 1, -1) * (double) f1;
    }

    private double p2f2() {
        int nTotalTracks = solutionInstance.getProblem().getTracks().size();
        //System.out.println("p2f2=" + Math.pow(nTotalTracks, -1) * solutionInstance.getNumOfUsedTracks());
        return Math.pow(nTotalTracks, -1) * solutionInstance.getNumOfUsedTracks();
  }
  ...
  private double p3f3() {
    ...
    private double r1g1() {
        int r1 = solutionInstance.getProblem().getVehicles().size() - solutionInstance.getNumOfUsedTracks();
        int g1 = 0;
        for (TrackInstance track : solutionInstance.getTrackInstances()) {
            boolean first = true;
            int previousType = -1;
    ...
    private double r2g2() {
        int r2 = solutionInstance.getNumOfUsedTracks() - 1;
        int g2 = 0;
        ...
        for (TrackInstance track : solutionInstance.getTrackInstances()) {
            if (!track.getParkedVehicles().isEmpty()) {
                if (first) {
                    first = false;
                    previousType = track.getParkedVehicles().get(track.getParkedVehicles().size() - 1).getVehicle().getLayoutType();
                } else {
